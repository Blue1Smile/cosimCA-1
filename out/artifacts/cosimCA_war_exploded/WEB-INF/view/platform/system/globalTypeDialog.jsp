...
function getAllSerialnum(){
	var url=__ctx+"/platform/system/indetity/getAllIndetity.ht";
...
function showResult(responseText){
	...
	if(isAdd){
		$.ligerMessageBox.confirm('提示信息',"添加成功，继续添加吗?",function(rtn){
			if(rtn){
				$("#typeName").val('');
				$("#nodeKey").val('');
			}
			else{
				$.ligerMessageBox.success('提示信息',"添加分类成功!",function(){
					window.close();
				});
			}
		});
		...
		$.ligerMessageBox.success('提示信息',"修改分类成功!",function(){
			window.close();
		});
	}
}
