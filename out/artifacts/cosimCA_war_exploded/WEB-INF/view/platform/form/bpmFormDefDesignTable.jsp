...
<script type="text/javascript">
	...
	function getJson(){
		...
		var str=JSON.stringify(mainJson);
		return str;
	};
	//返回编辑界面
	function goBack(){
	...
	<div class="panel-top">	
		...
		<div class="panel-toolbar">
			<div class="toolBar">
				...
				<div class="group">
					<a class="link back" href="javascript:;" onclick="goBack()">返回</a>
				</div>
			</div>
		</div>
				...
				<form id="bpmFormDefForm" action="designEdit.ht" method="post">
					<table cellpadding="0" cellspacing="0" border="0" style=" margin-bottom:4px;" >
						<tr style="height:25px;">
							...
							<td style="padding: 2px;">表名:&nbsp;<input id="tableName" type="text" <c:if test="${canEditTableName==false }">readonly='readonly'</c:if> name="tableName"  value="${tableName}" class="inputText" validate="{required:true,variable:true}" /></td>
							<td style="padding: 2px;">表描述:&nbsp;<input id="tableComment" type="text" name="tableComment" value="${tableComment}" class="inputText" /></td>
			...
			<table cellpadding="1" cellspacing="1" class="table-grid" >
				...
				<tbody  type="mainTable">
					...
					<tr fieldName="${field.fieldName}">
						...
						<td>
							<c:choose>
								<c:when test="${field.isAllowMobile==1}">
									<input type="checkbox" name="isAllowMobile" checked="checked" />
								</c:when>
								<c:otherwise>
									<input type="checkbox" name="isAllowMobile"  />
								</c:otherwise>
							</c:choose>
