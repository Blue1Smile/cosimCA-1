	...
	<script type="text/javascript">
		...
		function addRelation(){
			...
			if(trSource.length==0){
				$.ligerMessageBox.warn("请主表主键!");
				return;
			}
			if(trDest.length==0){
				$.ligerMessageBox.warn("请子表外键!");
				return;
			}
			...
			if(rtn==1){
				$.ligerMessageBox.warn(tableName + ",外键已经定义!");
				return ;
			}
			else if(rtn==2){
				$.ligerMessageBox.warn("主键已经定义为:" + relationObj.pk +",如果需要修改主键，请先删除所有的关联关系!");
				return ;
			}
		...
		function saveRelation(){
			...
			$.post(url,{relation:str,tablename:tablename,dataSource:dataSource},function(data){
				...
				if(obj.isSuccess()){//成功
					$.ligerMessageBox.success('提示信息','保存关联关系成功!',function(){
						window.close();
					});
				}
				else{
