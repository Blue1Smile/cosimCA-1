...
function save(){
	...
	if(aryId.length==0){
		$.ligerMessageBox.warn('提示信息',"没有定义按钮!");
		return;
	}
	var ids=aryId.join(",");
	var url="sort.ht";
	$.post(url,{ids:ids},function(responseText){
		...
		if(obj.isSuccess()){//成功
			$.ligerMessageBox.success('提示信息',"修改排序成功!",function(){
...
function init(){
	$("a.link.init").click(function(){
		var ele=this;
		$.ligerMessageBox.confirm('提示信息','确认初始化按钮吗？',function(rtn) {
			if(rtn){
		...
		<div class="panel-top">
			...
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<c:if test="${buttonFlag}"> 
						<div class="l-bar-separator"></div>
						<div class="group"><a  class="link back"  href="list.ht?defId=${defId}">返回</a></div>
					</c:if>
				</div>	
			</div>
		</div>
		<div class="panel-body">
			<table cellpadding="1" cellspacing="1" class="table-grid table-list">
				...
				<tr class="trNodeBtn" id="${item.id}">
						...
						<c:choose>
							<c:when test="${item.isstartform==1 }">
								<c:choose>
									...
									<c:when test="${item.operatortype==6 }">保存表单</c:when>
								</c:choose>
							</c:when>
							<c:otherwise>
								<c:choose>
									...
									<c:when test="${item.operatortype==12 }">短信</c:when>
									<c:when test="${item.operatortype==13 }">邮件</c:when>
								</c:choose>
							</c:otherwise>
						</c:choose>
					...
					<td>
						<a class="link edit" href="edit.ht?id=${item.id}&defId=${defId}&buttonFlag=${buttonFlag}">编辑</a>
