			...
			$("a.save").click(function() {
				...
				$('input[name="startTime"]').each(function(){
					...
					if(v.length!=5 || v.indexOf(':')==-1 || !isDigit(v.substring(0,2)) || !isDigit(v.substring(3,5))){
						$.ligerMessageBox.error('错误','开始时间输入格式有误',null);
						sflag = false;
					}
				});
				...
				if(sflag){
					$('input[name="endTime"]').each(function(){
						var v = $(this).attr('value');
						if(v.length!=5 || v.indexOf(':')==-1 || !isDigit(v.substring(0,2)) || !isDigit(v.substring(3,5))){
							$.ligerMessageBox.error('错误','结束时间输入格式有误',null);
							sflag = false;
						}
					});
				}
				...
				if(idx==0){
					$.ligerMessageBox.error('错误','没有添加班次时间',null);
				}
		...
		<div class="panel-body">
			...
			<form id="workTimeSettingForm" method="post" action="save.ht">
				...
				<div class="panel-toolbar">
					<div class="toolBar">
						<div class="group">
							<a class="link add" id="workTimeAdd" href="javascript:add()">添加</a></div>
							<div class="l-bar-separator"></div>
							<div class="group">
