...
<head>
	...
	<link href="${ctx}/styles/worktimeCalendar/calendar.css" rel="stylesheet" type="text/css" />
	<script type="text/javascript">
		...
		function changeSelItemWt(){
			...
			if(activeAmount==0){
				$.ligerMessageBox.warn('提示信息',"还没有任何选中项,请先选择日期!");
				return;
			}
			//班次类型
		...
		function noticeSave(flag,year,month){
 			...
 			if(hasModify){
 				$.ligerMessageBox.confirm('提示信息','要保存此月数据吗？',callback)	;
 			}
 			else{
 				jumpTo(flag,year,month);
 			}
	...
	<div class="panel-top">
		...
		<div class="panel-toolbar">
			<div class="toolBar">
				...
				<div class="l-bar-separator"></div>
				<div class="group"><a class="link next" id="nextMonth" href="javascript:noticeSave('next',${wtYear},${wtMon})">下月</a></div>
				<div class="l-bar-separator"></div>
				<div class="group">
					<form id="searchForm" method="post" action="edit.ht">
						<ul>
							<li style="float: left;"><span class="label">年份:</span></li>
							<li style="float: left;">
							...
							<li style="float: left;"><span class="label">月份:</span></li>
							<li style="float: left;">
			...
			<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
				<tr>
					<th style="text-align: left;">
						名称: <input type="text" size="10" id="name" name="name" value="${sysCalendar.name}" class="inputText"/>
						描述:<input type="text" id="memo" name="memo" value="${sysCalendar.memo}" class="inputText"/>
					</th>
					<th style="text-align: left;">
						班次：
						...
						<span style="color: red;font: 80%">点击日期选中进行修改</span>
						<a href='#' class='button'  onclick="changeSelItemWt()" ><span >修改选中项</span></a>
						<a href='#' class='button'  onclick="cleanSelItem()" ><span >清除选中项</span></a>
					</th>
				</tr>
			</table>
