...
<head>
	...
	<script type="text/javascript">
		 ...
		 */
		 function chooseUsers(a){
			var $table = $(a).parent().parent().parent();
			if( $table.find('input[name="taskName"]').val() == "4-会签" || $table.find('input[name="taskName"]').val() == "5-标准审查" || $table.find('input[name="taskName"]').val() == "6-批准" ){
      			alert("该环节只需选取审签企业");
      			return;
      		}
			//弹出用户选择器
			UserDialog({
				...
				callback:function(ids,names){
					if(ids=='' || ids==null || ids==undefined) return;
			...
			function _callBackUsers(userIds,userNames){
				alert(userIds);
			...
			function addMyEnt(a){
				var c_ent_id = '${resFilecheck.enterpriseId}';
				var c_ent_name = '${resFilecheck.enterpriseName}';
				var d_ent_id = $(a).closest('tr').prev().find('#en_id');
				var d_ent_name = $(a).closest('tr').find('#en_name');
				
				if($(a).attr('checked')){
					if(d_ent_id.val().indexOf(c_ent_id)==-1){
						if(d_ent_id.val()==''){//为空
							d_ent_id.val(c_ent_id);
							d_ent_name.val(c_ent_name);
						}else{//不为空
							d_ent_id.val(d_ent_id.val() + ',' + c_ent_id);
							d_ent_name.val(d_ent_name.val() + ',' + c_ent_name);
						}
					}
				}else{
					if(d_ent_id.val()!=''){
						var i = d_ent_id.val().indexOf(c_ent_id);
						var j = d_ent_name.val().indexOf(c_ent_name);
						var s = d_ent_id.val();
						var n = d_ent_name.val();
						if(i==0){//第一个
							d_ent_id.val('');
							d_ent_name.val('');
						}else if(i > 0){//最后一个:333,1
							s = s.substring(0,i-1);
							n = n.substring(0,j-1);
							d_ent_id.val(s);
							d_ent_name.val(n);
...
/*       	function _callBackMyFriends(friend){
      		$('.l-dialog-table  input[name="checkEnterpriseids"]').val(friend[0]);
      		$('.l-dialog-table  input[name="checkEnterprisenames"]').val(friend[1]);
      		entpriseChooser.close();
     
	} */
  	var rolesChooser;
  	function selRoleBySups(a){
  		...
  		rolesChooser = $.ligerDialog.open({ url:urlShow, height: 400,width: 800, title :'企业角色选择器', name:"frameDialog1_"});
  	}
  	function _callBackSelectedRoles(rows){
  		var checkerNames ="";
  		var checkerEntIds ="";
  		for(var k=0; k<rows.length; k++){
			var row = rows[k];
			checkerNames = checkerNames+row.companyName+"["+row.roleName+"];";
			if(k+1==rows.length){
				checkerEntIds = checkerEntIds+row.companyId;
			}
			else{
			checkerEntIds = checkerEntIds+row.companyId+",";
			}
  		...
  		checkerRoleIds = rows[0].roleId;
		$('.l-dialog-table  input[name="checkEnterpriseids"]').val(checkerEntIds);
  		$('.l-dialog-table  input[name="checkEnterprisenames"]').val(checkerNames);
  		// 使用用户ID记录所选企业的角色ID
  		$('.l-dialog-table  input[name="checkUserids"]').val(checkerRoleIds);
	...
	$(function(){
	 ...
	 var currentObject=null;
		...
		function fileCallback(fileId,fileName,filePath) {
			...
			var operaterId = ${resFilecheck.operatorId};
			var operaterName = '${resFilecheck.operatorName}';
			...
			formuploadUserid.val(operaterId);
			formuploadUsername.val(operaterName);
...
<div class="panel">
	<div class="panel-top">
		<div class="tbar-title">
		    <c:choose>
			    <c:when test="${resFilecheck.id !=null}">
			        <span class="tbar-label">编辑文档审签</span>
			    </c:when>
			    <c:otherwise>
			        <span class="tbar-label">添加文档审签</span>
			    </c:otherwise>			   
		    </c:choose>
		</div>
			...
			<table class="table-grid table-list" cellpadding="1" cellspacing="1" id="resFilecheckOpinion" formType="window" type="sub">
				...
				<c:forEach items="${resFilecheckOpinionList}" var="resFilecheckOpinionItem" varStatus="status">
				    <tr type="subdata">
				 	  ...
				 	  <!--<td style="text-align: center" name="checkUserids">${resFilecheckOpinionItem.checkUserids}</td>
					    <td style="text-align: center" name="checkEnterpriseids">${resFilecheckOpinionItem.checkEnterpriseids}</td>
	...
	<form id="resFilecheckDetailForm" style="display:none">
		<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
			<tr>
				<th width="20%">文件名: </th>
				<td><input type="text"  readonly name="fileName" value=""  class="inputText" validate="{required:false,maxlength:768}"/></td>
			</tr>
			<tr style="display: none;">
				<th width="20%">文件路径: </th>
			...
			<tr>
				<th width="20%">文件类型: </th>
				<td><input type="text" readonly name="fileType" value=""  class="inputText" validate="{required:false,maxlength:96}"/></td>
			</tr>
			<tr style="display: none;">
