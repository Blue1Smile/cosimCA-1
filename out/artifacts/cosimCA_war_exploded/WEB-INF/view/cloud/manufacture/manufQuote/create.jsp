		...
		<form id="manufQuoteForm" method="post" action="save.ht">
			<table class="table-detail" cellpadding="0" cellspacing="0" border="0" type="main">
				...
				<tr>
					<th width="20%">发布模式: </th>
					<td>
						<c:if test="${manufQuote.releaseModel == 'piblic'}">
							<input type="text" id="releaseModel" name="releaseModel" value="公开"  class="inputText" validate="{required:false,maxlength:96}"  readonly="readonly"/>
					    </c:if>
					    <c:if test="${manufQuote.releaseModel == 'please'}">
							<input type="text" id="releaseModel" name="releaseModel" value="邀请"  class="inputText" validate="{required:false,maxlength:96}"  readonly="readonly"/>
					    </c:if>
					</td>  
					<th width="20%">运费承担方: </th>
					<td>
						<c:if test="${ manufQuote.freightBearer == 'weituo'}">
						...
						<c:if test="${ manufQuote.freightBearer == 'shouli'}">
				...
				<tr>
					<th width="20%">运输方式: </th>
					<td>
						<c:if test="${manufQuote.transportWay=='haiyun'}">
					    ...
					    <c:if test="${manufQuote.transportWay=='kongyun'}">
					    ...
					    <c:if test="${manufQuote.transportWay=='luyun'}">
					...
					<th width="20%">币种: </th>
					<td>
						<c:if test="${manufQuote.currencyType == 'renminbi'}">
						...
						<c:if test="${manufQuote.currencyType == 'meiyuan'}">
				...
				<tr>
					...
					<th width="20%">是否带料: </th>
					<td>
						<c:if test="${ manufQuote.isBringmaterial == 1}">
						             带料加工
							<input type="hidden" id="isBringmaterial" name="isBringmaterial" value="${ manufQuote.isBringmaterial}"  class="inputText" validate="{required:false}" readonly="readonly" />
						</c:if>
						<c:if test="${ manufQuote.isBringmaterial == '0'}">
							自备材料
							<input type="hidden" id="isBringmaterial" name="isBringmaterial" value="${ manufQuote.isBringmaterial}"  class="inputText" validate="{required:false}" readonly="readonly" />
						</c:if>
					</td>
				...
				<tr>
					...
					<td>
						<c:if test="${ manufQuote.isReport == 1}">
						            需要汇报加工进度
							<input type="hidden" id="isReport" name="isReport" value="${manufQuote.isReport}"  class="inputText" validate="{required:false}"  />
						</c:if>
						<c:if test="${ manufQuote.isReport == 0}">
						           不 需要汇报加工进度
							<input type="hidden" id="isReport" name="isReport" value="${manufQuote.isReport}"  class="inputText" validate="{required:false}"  />
						</c:if>
					</td>
