...
<head>
	...
	<script type="text/javascript">
		...
		function fileCallback(fileIds,fileNames,filePaths)
		{
			...
			var operaterId = ${crowdsourcingAgreement.operaterId};
			var operaterName = '${crowdsourcingAgreement.operaterName}';
			
			//var downurl = "${ctx}/platform/system/sysFile/download.ht?fileId="+fileIds;
			
			attachmentName.prepend(""+fileNames+"&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"javascript:getFileInfo('${ctx}',"+fileIds+");\">查看信息</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"javascript:openFile('${ctx}',"+fileIds+");\">在线浏览</a>");
		...
		function add_attachment(){
	    	...
	    	$("#crowdsourcingAgreementDetail").append(row);
		}
		 /**
		*全选/反选
		*/
...
<!-- 					<th width="20%">研发物品ID: </th> -->
			...
			<table class="table-grid table-list" cellpadding="1" cellspacing="1" id="crowdsourcingAgreementDetail" formType="window" type="sub">
				...
				<c:forEach items="${crowdsourcingAgreementDetailList}" var="crowdsourcingAgreementDetailItem" varStatus="status">
				    <tr type="subdata">
				   		<td style="text-align: center"><input type="checkbox" name="check" value=""/></td>
				    	...
				    	<c:if test="${crowdsourcingAgreementDetailItem.operatingAuthority==2}">
				    		<td style="text-align: center" name="attachmentName">${crowdsourcingAgreementDetailItem.attachmentName}&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:getFileInfo('${ctx}',${crowdsourcingAgreementDetailItem.attachmentManageId});">查看信息</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:openFile('${ctx}',${crowdsourcingAgreementDetailItem.attachmentManageId});">在线浏览</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:downloadFile('${ctx}',${crowdsourcingResponseDetailItem.attachmentManageId});">下载</a></td>
				    	</c:if>
					    ...
					    <td style="text-align: center;display:none;" name="uploadId">${crowdsourcingAgreementDetailItem.uploadId}</td>
					    <td style="text-align: center" name="uploadName">${crowdsourcingAgreementDetailItem.uploadName}</td>
					    <td style="text-align: center;display:none;" name="attachmentManageId">${crowdsourcingAgreementDetailItem.attachmentManageId}</td>
					    <td style="text-align: center" name="operatingAuthority">
					    	<c:if test="${crowdsourcingAgreementDetailItem.operatingAuthority==0}">查看信息</c:if>
					    	<c:if test="${crowdsourcingAgreementDetailItem.operatingAuthority==1}">在线浏览</c:if>
					    	<c:if test="${crowdsourcingAgreementDetailItem.operatingAuthority==2}">下载</c:if>
