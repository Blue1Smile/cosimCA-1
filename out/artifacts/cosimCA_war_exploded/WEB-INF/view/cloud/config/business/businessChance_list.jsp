...
<body>
	...
	<div id="chanceList01">
		...
		<div id="model01">
			...
			<c:forEach items="${businessChanceList}" var="businessChance"
				varStatus="s">
				<div class="div01_1">
					<div class="div01_2">
						<div class="div01_3">
							<img src="${ctx}${businessChance.image}" width="100" height="100"
								onError="this.src='${ctx}/images/default-chance.jpg';" />
						</div>
						<div class="div01_4">
							${businessChance.name}
							<span class="grey2">${businessChance.info.name}</span>
						</div>
						<div class="div01_5">
							<span class="grey3">开始时间：<fmt:formatDate
									value="${businessChance.startTime}" pattern="yyyy-MM-dd" />
								截止时间：<fmt:formatDate value="${businessChance.endTime}"
									pattern="yyyy-MM-dd" /> <br> <c:if
									test="${businessChance.type=='生产商机'}">
                            	 生产要求：${businessChance.properties} 
                            	 </c:if> <c:if
									test="${businessChance.type=='采购商机'}">
                            	 采购数量：${businessChance.properties} 
                            	 </c:if> <c:if
									test="${businessChance.type=='营销商机'}">
                            	 代理区域：${businessChance.properties} 
                            	 </c:if> <c:if
									test="${businessChance.type=='服务商机'}">
                            	 服务区域：${businessChance.properties} 
                            	 </c:if> <c:if
									test="${businessChance.type=='研发商机'}">
                            	 研发要求：${businessChance.properties} 
                            	 </c:if> </br>商机类型：${businessChance.type} </br>${businessChance.info.country}：${businessChance.info.province}${purchaseList.info.city}
							</span>
						</div>
