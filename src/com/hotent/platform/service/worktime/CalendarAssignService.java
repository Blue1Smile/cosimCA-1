...
public class CalendarAssignService extends BaseService<CalendarAssign>
{
				...
				for(WorkTime workTime:worklist){
					...
					if(start.compareTo(startWork)<=0){
						...
						if(end.compareTo(startWork)>=0&&end.compareTo(endWork)<=0){
							...
							System.out.println(workTime.getCalDay()+",请假了。。。");
						}
	...
	public Date getRelativeStartTime(String actInstanceId,String nodeId,Integer eventType){
		List<HistoricTaskInstanceEntity> historicTaskInstanceEntitys = (List<HistoricTaskInstanceEntity>) historyTaskInstanceDao.getByInstanceIdAndNodeId(actInstanceId, nodeId);
		if(historicTaskInstanceEntitys!=null){
			if(new Integer(0).equals(eventType)){
				Date startTime = null;
				//获取子历史任务最早开始时间
				for(HistoricTaskInstanceEntity historicTaskInstanceEntity : historicTaskInstanceEntitys){
					if(startTime == null){
						startTime = historicTaskInstanceEntity.getStartTime();
					}
					if(historicTaskInstanceEntity.getStartTime()!=null && (historicTaskInstanceEntity.getStartTime().getTime() < startTime.getTime())){
						startTime = historicTaskInstanceEntity.getStartTime();
					}
				}
				return startTime;
				...
				for(HistoricTaskInstanceEntity historicTaskInstanceEntity : historicTaskInstanceEntitys){
					if(endTime == null){
						endTime = historicTaskInstanceEntity.getEndTime();
					}
					if(historicTaskInstanceEntity.getEndTime()!=null && (historicTaskInstanceEntity.getEndTime().getTime() < endTime.getTime())){
						endTime = historicTaskInstanceEntity.getEndTime();
					}
				}
				return endTime;
