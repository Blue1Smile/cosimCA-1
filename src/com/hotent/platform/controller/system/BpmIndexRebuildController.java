...
{	
	...
	public ModelAndView indexList(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		if(dbType.equals(SqlTypeConst.ORACLE)||dbType.equals(SqlTypeConst.SQLSERVER)||dbType.equals(SqlTypeConst.DB2)||dbType.equals(SqlTypeConst.H2)||dbType.equals(SqlTypeConst.DM)){
		...
		}else if(dbType.equals(SqlTypeConst.MYSQL)){//与Oracle等数据库不同，MySQL的索引重建是通过对表的Alter原语进行操作的。
			Map<String,String> tables=tableMeta.getTablesByName(tableName);
			for(Entry<String, String> entry:tables.entrySet()){
				Map<String,Object> map=new HashMap<String, Object>();
				map.put("tableName",entry.getKey());
				map.put("tableComment",entry.getValue());
				map.put("isCustom", isCustomTable(entry.getKey()));
				indexes.add(map);
			}
		}
	...
	private Map<String,String> getTablesCommentByNames(List<String> tableNames){
