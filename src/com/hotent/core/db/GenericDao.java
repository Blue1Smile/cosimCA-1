...
import com.hotent.platform.service.compass.SqlMapClientGpsDevice;
...
public abstract class GenericDao<E, PK extends Serializable> extends
		BaseMyBatisDao implements IEntityDao<E, PK> {
	...
	@Resource
	protected SqlMapClientGpsDevice mybatisGpsDevice;
	...
	public E getById(PK primaryKey) {
		...
		E object = (E) getSqlSessionTemplate().selectOne(getStatement,
				primaryKey);
	 ...
	 * 
	 * @param sqlKey
	 *            <a href="#Mapper键值ID">Mapper键值ID</a>。
	 * @param params
	 *            参数为单个对象或Map类型
	 * @return
	 */
	 ...
	 * @param params
	 *            参数 参数为单个对象或Map类型
	 *            ,若要排序，请在该参数Map里加上两参数key为orderField与orderSeq的值对。
	 * @param pb
	 *            分页对象 {@link PageBean pageBean}
	 * @return
	 */
	 ...
	 *            <a href="#Mapper键值ID">Mapper键值ID</a>
	 * @param params
	 *            参数类型 可为Object类型，也可为Map类型,若要排序，
	 *            请在该参数Map里加上两参数key为orderField与orderSeq的值对。
	 * @return
	 */
	...
	/**
	 * 按Map.xml中的select指定的id获取数据列表。 没有参数。
	 * 
	 * @param sqlKey
	 *            <a href="#Mapper键值ID">Mapper键值ID</a>
	 * @return
	...
	public int delById(PK id) {
		...
		// xyy 比较当前类是否需要建立索引
		try{
			for (int i = 0; i < mybatisGpsDevice.getIndexStatements().length; i++) {
	
				if (mybatisGpsDevice.getIndexStatements()[i]
						.equals(getIbatisMapperNamespace() + ".getAllForCompass")) {
					mybatisGpsDevice.getGps().index(getEntityClass());
				}
			}
		}catch(Throwable t){
			System.out.println("索引建立失败" + t.getMessage());
		}
		// -------------------------------------
	...
	public void add(E entity) {
		...
		// xyy 比较当前类是否需要建立索引
		try{
			for (String statement : mybatisGpsDevice.getIndexStatements()) {	
				System.out.println("=====" + getIbatisMapperNamespace() + ".getAllForCompass");
				if (statement.equals(getIbatisMapperNamespace() + ".getAllForCompass")) {
					mybatisGpsDevice.getGps().index(getEntityClass());
				}
			}
		}catch(Throwable t){
	...
	public List<E> getList(String statementName, Object params,
		...
		if (pageBean.isShowTotal()) {
			...
			// 取得返回数量
			int totalCount = jdbcTemplate.queryForInt(ibatisSql.getCountSql(),
					ibatisSql.getParameters());

			pageBean.setTotalCount(totalCount);
		}
		List<E> list = getSqlSessionTemplate().selectList(statementName,
				filters, pageBean.getFirst(), pageBean.getPageSize());
		return list;
	}
