...
//    }
    ...
    public void showprivatedata(HttpServletRequest request, HttpServletResponse response) throws Exception {
        try {
            ...
            //任务所有的私有数据
            List<PrivateData> privateDataListbyTask = privateDataService.queryPrivateDataByddTaskID(taskId);
            ...
            for (int i = 0; i < privateDataListbyTask.size(); i++) {
                tempPrivateData = privateDataListbyTask.get(i);
            ...
            json.put("total", privateDataListbyTask.size());
    ...
    public void canbeorderprivatedata(HttpServletRequest request, HttpServletResponse response) throws Exception {
        try {
            ...
            //当前任务私有数据列表
            List<PrivateData> taskPrivatedataList = privateDataService.queryPrivateDataByddTaskID(taskId);
            //根据taskid数据任务订阅关系列表
            ...
            for (int i = 0; i < orderDataRelationList.size(); i++) {
                OrderDataRelation orderDataRelation = orderDataRelationList.get(i);
                    PrivateData privateData = privateDataService.getById(orderDataRelation.getDdDataId());
                    OrderPrivatedataList.add(privateData);
            ...
            //保证任务本身的私有数据不被订阅
            for(int i = 0; i<taskPrivatedataList.size();i++){
                OrderPrivatedataList.add(taskPrivatedataList.get(i));
            }
            //获取任务可订阅数据列表
