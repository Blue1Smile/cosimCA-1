...
function alertConditionSn(curObj,otherObj,direct){
	...
	if(currentId.length>0 && otherId.length>0){
		$.post('${ctx}/platform/bpm/bpmUserCondition/updateSn.ht',{'currentId':currentId,'otherId':otherId,'currentSn':otherSn,'otherSn':currentSn},function(data){
			...
			if(resultData.result==1){
				...
				$.ligerMessageBox.warn(resultData.message);
			}
		});
	}
}
...
function delRows(tableId){
	...
	for(var i=deletes.length-1;i>=0;i--){
		...
		if(conditionId!='' && conditionId!=undefined){
			$.post('${ctx}/platform/bpm/bpmUserCondition/del.ht',{'id':conditionId});
		}
...
function conditionDialog(tableId,edit)
{
	...
	var url;
	if(conditionId){
		url=__ctx + '/platform/bpm/bpmDefinition/conditionEdit.ht?defId='+defId+'&nodeTag='+nodeId+'&conditionId='+conditionId;
	}	
	else if(edit && !conditionId){
		$.ligerMessageBox.warn("请先选择要修改的规则条件！");
		return;
	}else{
		url=__ctx + '/platform/bpm/bpmDefinition/conditionEdit.ht?defId='+defId+'&nodeTag='+nodeId;
	}
...
function repairOldData(tableId){
	...
	$.post(__ctx+"/platform/bpm/bpmUserCondition/save.ht",postData,function(response){
		...
		if(resultJson.result==1){		
			...
			$.post(__ctx+"/platform/bpm/bpmDefinition/repairData.ht", {'conditionId':conditionId,'setId':setId},function(data){
