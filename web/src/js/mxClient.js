...
if(state.shape!=null)
{
var shape=(state.shape.stencil!=null)?state.shape.stencil:
...
{
return{
isExportWrapperShape:true,drawShape:function(canvas,state,bounds,background)
{
var x0=bounds.x;
var y0=bounds.y;
var path=
...
canvas.text(bounds.x,bounds.y,bounds.width,bounds.height,str,align,null,vert);
};
...
var unitX=dx/dist;
var unitY=dy/dist;
var size=mxUtils.getValue(state.style,(source)?mxConstants.STYLE_STARTSIZE:
...
pe=pe.clone();
var type=mxUtils.getValue(state.style,(source)?mxConstants.STYLE_STARTARROW:
...
var state=
{
alpha:1,dashed:false,strokewidth:1,fontsize:mxConstants.DEFAULT_FONTSIZE,fontfamily:mxConstants.DEFAULT_FONTFAMILY,fontcolor:'#000000'
};
var f2=function(x)
{
return Math.round(parseFloat(x)*100)/100;
};
var updateImageBaseUrl=function()
{
imageBaseUrl=document.URL;
var tmp=imageBaseUrl.lastIndexOf('/');
if(tmp>0)
{
imageBaseUrl=imageBaseUrl.substring(0,tmp+1);
}
};
var convertUrl=function(src)
{
...
return{
isAbsoluteImageUrls:function()
{
return absoluteImageUrls;
},
setAbsoluteImageUrls:function(value)
{
absoluteImageUrls=value;
},
...
var currentState=
{
dx:0,dy:0,scale:1,transform:'',fill:null,gradient:null,stroke:null,strokeWidth:1,dashed:false,dashpattern:'3 3',alpha:1,linecap:'flat',linejoin:'miter',miterlimit:10,fontColor:'#000000',fontSize:mxConstants.DEFAULT_FONTSIZE,fontFamily:mxConstants.DEFAULT_FONTFAMILY,fontStyle:0
};
...
var anchor=(align==mxConstants.ALIGN_RIGHT)?'end':(align==mxConstants.ALIGN_CENTER)?'middle':
'start';
if(anchor=='end')
{
x+=Math.max(0,w-2);
...
this.guideX.points=[new mxPoint(x,0),new mxPoint(x,c.scrollHeight-2)];
}
...
var c=this.graph.container;
this.guideY.points=[new mxPoint(0,y),new mxPoint(c.scrollWidth-2,y)];
}
...
var points=moveCmd+' '+Math.round(p0.x-offsetX)+' '+Math.round(p0.y-offsetY)+' ';
...
points+=lineCmd+' '+Math.round(pt.x+nx1-offsetX)+' '+Math.round(pt.y+ny1-offsetY)+' ';
...
points+=curveCmd+' '+Math.round(pt.x-offsetX)+' '+Math.round(pt.y-offsetY)+' '+Math.round(pt.x-offsetX)+','+Math.round(pt.y-offsetY)+' '+Math.round(pt.x+nx2-offsetX)+' '+Math.round(pt.y+ny2-offsetY)+' ';
}
}
else
{
points+=lineCmd+' '+Math.round(pt.x-offsetX)+' '+Math.round(pt.y-offsetY)+' ';
}
}
return points;
};
...
node.setAttribute('x',Math.round(this.bounds.x)+crisp);
node.setAttribute('y',Math.round(this.bounds.y)+crisp);
...
if(this.shadowNode!=null)
{
this.shadowNode.setAttribute('transform',mxConstants.SVG_SHADOWTRANSFORM+' '+transform);
}
}
}
else
...
{
fill:shape.fill,stroke:shape.stroke,strokeWidth:(this.strokewidth=='inherit')?Number(shape.strokewidth)*shape.scale:
Number(this.strokewidth)*minScale,dashed:shape.isDashed,dashpattern:phase+' '+phase,alpha:shape.opacity,linejoin:'miter',fontColor:'#000000',fontSize:mxConstants.DEFAULT_FONTSIZE,fontFamily:mxConstants.DEFAULT_FONTFAMILY,fontStyle:0
};
var currentPath=null;
...
{
var op=(vml)?'m':'M';
this.lastMoveX=Math.round(x0+Number(node.getAttribute('x'))*sx);
this.lastMoveY=Math.round(y0+Number(node.getAttribute('y'))*sy);
...
{
this.lastMoveX=Math.round(x0+Number(node.getAttribute('x2'))*sx);
this.lastMoveY=Math.round(y0+Number(node.getAttribute('y2'))*sy);
addToPath('Q '+Math.round(x0+Number(node.getAttribute('x1'))*sx)+' '+Math.round(y0+Number(node.getAttribute('y1'))*sy)+' '+this.lastMoveX+' '+this.lastMoveY);
}
}
else if(name=='curve')
{
var op=(vml)?'c':'C';
this.lastMoveX=Math.round(x0+Number(node.getAttribute('x3'))*sx);
this.lastMoveY=Math.round(y0+Number(node.getAttribute('y3'))*sy);
addToPath(op+' '+Math.round(x0+Number(node.getAttribute('x1'))*sx)+' '+Math.round(y0+Number(node.getAttribute('y1'))*sy)+' '+Math.round(x0+Number(node.getAttribute('x2'))*sx)+' '+Math.round(y0+Number(node.getAttribute('y2'))*sy)+' '+this.lastMoveX+' '+this.lastMoveY);
}
else if(name=='close')
{
addToPath((vml)?'x':'Z');
...
var x=Math.round(x0+Number(node.getAttribute('x'))*sx);
var y=Math.round(y0+Number(node.getAttribute('y'))*sy);
var w=Math.round(Number(node.getAttribute('w'))*sx);
var h=Math.round(Number(node.getAttribute('h'))*sy);
...
shadow.setAttribute('offset',mxConstants.SHADOW_OFFSET_X+'px,'+mxConstants.SHADOW_OFFSET_Y+'px');
...
shadow.setAttribute('stroke',mxConstants.SHADOWCOLOR);
shadow.setAttribute('fill',mxConstants.SHADOWCOLOR);
shadow.setAttribute('transform','translate('+mxConstants.SHADOW_OFFSET_X+' '+mxConstants.SHADOW_OFFSET_Y+') '+(shadow.getAttribute('transform')||''));
shadow.setAttribute('opacity',mxConstants.SHADOW_OPACITY);
...
var sw=(this.strokewidth=='inherit')?Number(mxUtils.getNumber(state.style,mxConstants.STYLE_STROKEWIDTH,1))*state.view.scale:
...
if(isVml)
{
node.path='m'+Math.round(pe.x)+','+Math.round(pe.y)+' l'+Math.round(pe.x-nx-ny/2)+' '+Math.round(pe.y-ny+nx/2)+((type!=mxConstants.ARROW_CLASSIC)?'':
' '+Math.round(pe.x-nx*3/4)+' '+Math.round(pe.y-ny*3/4))+' '+Math.round(pe.x+ny/2-nx)+' '+Math.round(pe.y-ny-nx/2)+' x e';
}
else
{
node.setAttribute('d','M '+pe.x+' '+pe.y+' L '+(pe.x-nx-ny/2)+' '+(pe.y-ny+nx/2)+((type!=mxConstants.ARROW_CLASSIC)?'':
...
var x=this.bounds.x;
var y=this.bounds.y;
x+=(this.align==mxConstants.ALIGN_RIGHT)?((this.horizontal)?this.bounds.width:this.bounds.height)-this.spacingRight*this.scale:(this.align==mxConstants.ALIGN_CENTER)?this.spacingLeft+(((this.horizontal)?this.bounds.width:this.bounds.height)-this.spacingLeft-this.spacingRight)/2:
this.spacingLeft*this.scale+1;
y+=(this.valign==mxConstants.ALIGN_BOTTOM)?((this.horizontal)?this.bounds.height:this.bounds.width)-(lineCount-1)*dy-this.spacingBottom*this.scale-4:(this.valign==mxConstants.ALIGN_MIDDLE)?(this.spacingTop*this.scale+((this.horizontal)?this.bounds.height:this.bounds.width)-this.spacingBottom*this.scale-(lineCount-1.5)*dy)/2:
...
this.backgroundNode.setAttribute('y',this.boundingBox.y+this.scale/2+2*this.scale);
...
else if(this.imageNode.nodeName!='DIV')
{
...
this.start.coordsize=this.node.coordsize;
...
this.end.coordsize=this.node.coordsize;
...
var size=(this.graph.isSwimlane(parent))?this.graph.getStartSize(parent):
new mxRectangle();
...
{
ORTHOGONAL:1,POLYLINE:2,STRAIGHT:3
};
...
if(!graph.isHtmlLabel(state.cell)&&!mxUtils.isNode(value)&&mxClient.IS_IE&&document.documentMode!=9&&value!=null)
{
...
{
var result=state;
if((mxClient.IS_IE&&mxEvent.getSource(evt).nodeName=='IMG')||mxClient.IS_TOUCH)
{
var x=mxEvent.getClientX(evt);
var y=mxEvent.getClientY(evt);
...
{
var edgeStyle=(source!=null&&source==target)?mxUtils.getValue(edge.style,mxConstants.STYLE_LOOP,this.graph.defaultLoopStyle):(!mxUtils.getValue(edge.style,mxConstants.STYLE_NOEDGESTYLE,false)?edge.style[mxConstants.STYLE_EDGE]:
null);
...
border+=parseFloat(edge.style[(source)?mxConstants.STYLE_SOURCE_PERIMETER_SPACING:
...
if(this.isContainerEvent(evt)&&((!mxClient.IS_IE&&!mxClient.IS_GC&&!mxClient.IS_OP&&!mxClient.IS_SF)||!this.isScrollEvent(evt)))
{
...
{
if(terminal!=null&&terminal.shape!=null&&terminal.shape.constructor==mxStencilShape)
{
...
{
var point=null;
var x=edge.style[(source)?mxConstants.STYLE_EXIT_X:
mxConstants.STYLE_ENTRY_X];
if(x!=null)
{
var y=edge.style[(source)?mxConstants.STYLE_EXIT_Y:
...
{
return(state!=null&&state.style!=null)?(state.style[mxConstants.STYLE_VERTICAL_ALIGN]||mxConstants.ALIGN_MIDDLE):
null;
};
...
if(!this.isEditing()&&(mxClient.IS_OP||mxClient.IS_SF||mxClient.IS_GC||(mxClient.IS_IE&&mxClient.IS_SVG)||me.getEvent().target!=this.container))
{
...
mxEvent.addListener(this.sizer.node,(mxClient.IS_TOUCH)?'touchstart':'mousedown',mxUtils.bind(this,function(evt)
{
...
{
var sizer=new mxRectangleShape(this.bounds,mxConstants.OUTLINE_HANDLE_FILLCOLOR,mxConstants.OUTLINE_HANDLE_STROKECOLOR);
...
Math.max(1,this.bounds.width+dx),Math.max(1,this.bounds.height+dy));
this.selectionBorder.bounds=bounds;
this.selectionBorder.redraw();
}
var s=this.sizerSize;
...
var tmp=(this.graph.isSwimlane(current))?this.graph.getStartSize(current):
new mxRectangle();
...
var tmp=(this.graph.isSwimlane(swimlane))?this.graph.getStartSize(swimlane):
new mxRectangle();
w-=tmp.width;
h-=tmp.height;
...
{
return(this.connectImage==null||this.isConnecting())?mxCellMarker.prototype.getMarkerColor.apply(marker,arguments):
null;
});
...
{
icon.dialect=(this.graph.dialect==mxConstants.DIALECT_SVG)?mxConstants.DIALECT_SVG:
...
Math.abs(this.initialTouchX-me.getX())<this.tapAndHoldTolerance&&Math.abs(this.initialTouchY-me.getY())<this.tapAndHoldTolerance;
}
...
{
var next=(this.waypoints!=null&&this.waypoints.length>0)?this.waypoints[this.waypoints.length-1]:
...
hl.dialect=(this.graph.dialect==mxConstants.DIALECT_SVG)?mxConstants.DIALECT_SVG:
mxConstants.DIALECT_VML;
...
if(this.sizers!=null)
{
var pt=(this.allowHandleBoundsCheck&&mxClient.IS_IE)?new mxPoint(me.getGraphX(),me.getGraphY()):null;
for(var i=0;i<this.sizers.length;i++)
{
if(me.isSource(this.sizers[i])||(pt!=null&&this.sizers[i].node.style.visibility!='hidden'&&mxUtils.contains(this.sizers[i].bounds,pt.x,pt.y)))
{
return i;
}
}
...
if(this.bends!=null)
{
var pt=(mxClient.IS_IE&&this.allowHandleBoundsCheck)?new mxPoint(me.getGraphX(),me.getGraphY()):null;
for(var i=0;i<this.bends.length;i++)
{
if(me.isSource(this.bends[i])||(pt!=null&&this.bends[i].node.style.visibility!='hidden'&&mxUtils.contains(this.bends[i].bounds,pt.x,pt.y)))
{
return i;
}
}
...
mxEditor.prototype.propertiesResource=(mxClient.language!='none')?'任务属性':'';
mxEditor.prototype.tasksResource=(mxClient.language!='none')?'项目模板':'';
...
mxEditor.prototype.taskinfo=null;
...
mxEditor.prototype.propertiesWidth=200;
...
//@zx这里调试了一下。
//window.console.log(xml.documentElement);
...
mxEditor.prototype.open_new=function(filename)
{
if(filename!=null)
{
var xml=mxUtils.load(filename).getXml();
//@zx这里调试了一下。
//window.console.log(xml.documentElement);
this.readGraphModel_new(xml.documentElement);
this.filename=filename;
this.fireEvent(new mxEventObject(mxEvent.OPEN,'filename',filename));
}
};
...
//@zx大刀破斧试验啦
mxEditor.prototype.readGraphModel_exp=function(node)
{
...
var result = this.graph.getExportableCells(this.graph.getSelectionCells());

mxClipboard.insertCount = 1;
mxClipboard.cells = this.graph.cloneCells(result);


var cells = null;
var xml3=mxUtils.load('diagrams/cells.xml').getXml();
//window.console.log(xml3);
var dec2=new mxCodec(xml3.documentElement.ownerDocument);

dec2.decode(xml3.documentElement,this.graph.getModel());
...
//@zx大刀破斧试验啦
mxEditor.prototype.readGraphModel_exp2=function(node)
{

};

//@zx大刀破斧试验啦
mxEditor.prototype.readGraphModel_new=function(node)
{
    var project_id =  this.graph.getDefaultParent().getAttribute('projectId');
...
var cells = this.graph.getImportableCells(mxClipboard.cells);
var delta = mxClipboard.insertCount * mxClipboard.STEPSIZE;
var parent = this.graph.getDefaultParent();
...
mxClipboard.insertCount++;
    ...
    this.graph.getDefaultParent().setAttribute('projectId',project_id);
    var enc=new mxCodec();
    window.console.log(mxUtils.getPrettyXml(enc.encode(this.graph.getModel())));
...
if(node!=null)
{
	this.properties=new mxWindow(mxResources.get(this.propertiesResource)||this.propertiesResource,node,document.body.clientWidth-220,560,this.propertiesWidth,this.propertiesHeight,false);
...
//@zx 新增一个任务信息的面板试试看
mxEditor.prototype.showTaskinfo=function()
{
...
var wnd=new mxWindow("任务模板",div,w-220,280,200);
...
mxEditor.prototype.refreshTaskinfo=function(div)
{
if(this.taskinfo!=null)
{
var div=this.taskinfo.content;
mxEvent.release(div);
div.innerHTML='';
this.createTaskinfo(div);
}
...
mxEditor.prototype.editID=function()
{
    this.graph.getDefaultParent().setAttribute('projectId','12345');
    var enc=new mxCodec();
   // window.console.log(mxUtils.getPrettyXml(enc.encode(this.graph.getModel())));
