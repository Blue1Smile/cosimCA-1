	...
	$(document).ready(function() {
		...
		$("#dataFormSave").click(function() {
			...
			if (idxName.indexOf(' ') >= 0) {
				$.ligerMessageBox.warn("索引名不能包含空格！");
				return false;
			}
	...
	<form id="frmIndex" action="">
		...
		<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
			...
			<tr>
				...
				<td colspan="3"><select id="indexType" name="indexType" value="${index.indexType }">
						<c:choose>
							<c:when test="${dbType=='oracle'} ">
								<option value="BTREE">B-Tree索引</option>
								<option value="BITMAP">位图索引</option>
							</c:when>
							<c:when test="${dbType=='mysql'}">
								<option value="BTREE">B-Tree索引</option>
							</c:when>
							<c:when test="${dbType=='sqlserver'}">
								<option value="NONCLUSTERED">非聚集索引</option>
							</c:when>
							<c:when test="${dbType=='db2'}">
								<option value="NONCLUSTERED">非聚集索引</option>
								<option value="CLUSTERED">聚集索引</option>
							</c:when>
							<c:when test="${dbType=='h2'}">
								<option value="BTREE">B-Tree索引</option>
							</c:when>
							<c:when test="${dbType=='dm'}">
								<option value="BTREE">B-Tree索引</option>
								<option value="BITMAP">位图索引</option>
							</c:when>
						</c:choose>
