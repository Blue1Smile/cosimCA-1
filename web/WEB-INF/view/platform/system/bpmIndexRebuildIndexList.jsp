				...
				buttons : [ {
					...
					onclick : function(button,dialog){
						...
						if ($.trim(jobName) != '') {
							...
							$.ligerMessageBox.warn("索引重建计划名不能为空");
...
// 			var table = $(this).next('input[name="indexTable"]').val();
		...
		var params={
				indexes:JSON.stringify(indexAry),
				jobName:$('#jobName').val(),
	...
	<div class="panel">
		<div class="panel-top">
			...
			<div class="panel-search">
				<form id="searchForm">
					<div class="row">
			            <span class="label">索引名称:</span><input type="text" name="Q_indexName_S"  class="inputText" value="${param['Q_indexName_S']}"/>
			            <span class="label">表名称:</span><input type="text" name="Q_tableName_S"  class="inputText" value="${param['Q_tableName_S']}"/>
					</div>
				</form>
			</div>
		</div>
						...
						<table id="indexList" class="table-grid table-list" cellpadding="1" cellspacing="1">
					   		...
					   		<tbody>
					   			<c:forEach items="${indexList}" var="indexMap">
					   				<tr>
					   					<td width="100px">
					   						<input class="pk"  type="checkbox" value="" 
					   						table="${indexMap['tableName']}"
					   						custom="${indexMap['isCustom'] }"
					   						primaryKeyIndex=""
					   						/>
					   					</td>
					   					<td >${indexMap['tableName']}</td>
					   					<c:choose>
					   						<c:when test="${indexMap['isCustom'] }">
					   							<td >√</td>
					   						</c:when>
	...
	<div id="divJob" style="display:none">
  		<form id="frmJob" >
  			<table class="table-grid table-list">
  				...
  				<tr>
  					<td>描述:</td>
  					<td><textarea id="jobDescription1" cols="30" rows="7"></textarea></td>
  				</tr>
  			</table>
  		</form>
  	</div>
