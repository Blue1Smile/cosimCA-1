...
<head>
	...
	<script type="text/javascript">
		...
		function addNew(parentId,typeId,returnUrl)
		{
			if(parentId==0)
			{
				 $.ligerMessageBox.warn("请选择左边具体的分类才能添加数据字典!");
				 return;
			}
			else
			{
				...
				<div class="panel-body">
					<div class="panel-search">
							<form id="searchForm" method="post" action="list.ht">
									<div class="row">
												<span class="label" style="display:none; ">nodePath:</span><input type="hidden" name="nodePath"  class="inputText" value="${nodePath }" readonly="readonly"/>
											
												<span class="label">项Key:</span><input type="text" name="itemKey"  class="inputText" value="${param['itemKey']}"/>
											
												<span class="label">项名:</span><input type="text" name="itemName"  class="inputText" value="${param['itemName']}"/>
