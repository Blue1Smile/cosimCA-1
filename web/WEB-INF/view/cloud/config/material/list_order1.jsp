...
function toInterestStrategy(materialId){
	var currentEntid = $("#friends").val();
	if(currentEntid==""){
		window.location="${ctx}/cloud/config/interestStrategy/listInterest.ht?currentEntid=&materialId="+materialId;
	}else{
		window.location="${ctx}/cloud/config/interestStrategy/listInterest.ht?currentEntid="+currentEntid+"&materialId="+materialId;
	}
	
}
function viewInterestStrategy(materialId){
	var currentEntid = $("#friends").val();
	if(currentEntid==""){
		window.location="${ctx}/cloud/config/interestStrategy/listInterest.ht?currentEntid=&materialId="+materialId+"&type=view";
	}else{
		window.location="${ctx}/cloud/config/interestStrategy/listInterest.ht?currentEntid="+currentEntid+"&materialId="+materialId+"&type=view";
	}
...
<body>
	<div class="panel">
		...
		<div class="panel-body">
		    ...
		    <display:table name="materialList" id="materialItem" requestURI="list_order1.ht" sort="external" cellpadding="1" cellspacing="1" export="true"  class="table-grid">
				...
				<display:column title="管理" media="html" style="width:180px;white-space:nowrap">
					<c:if test="${materialItem.type == '1' }">		
							...
							<c:if test="${materialItem.sharedRepository == '1' }">
								<a href="${ctx}/cloud/warehouse/warehouseAccounts/productNums.ht?id=${materialItem.id}" class="link detail" name="saleRepository" >代理商商库存查询</a>
							</c:if>
							<a href="#" onclick="toInterestStrategy(${materialItem.id})" class="link detail">代理商返利策略管理</a>
					</c:if>
					<c:if test="${materialItem.type == '0' }">
						...
						<a href="#" onclick="viewInterestStrategy(${materialItem.id})" class="link detail">返利折扣查询</a>		
					</c:if>			
