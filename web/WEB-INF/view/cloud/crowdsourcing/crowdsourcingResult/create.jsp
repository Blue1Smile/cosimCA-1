...
// 	};
	...
	function fileCallback(fileIds,fileNames,filePaths,ext)
	{
		...
		for(var i=0;i<arrfileName.length;i++){
			if(i==0){
				temp = arrfileName[i]+"."+extname[i];
			}else{
				temp = temp+","+arrfileName[i]+"."+extname[i];
			}
		}
		link2 = temp;
		for(var i=0;i<arrfileName.length;i++){
			var tr=getRow(arrfileId[i],arrPath[i],arrfileName[i]+"."+extname[i]);
			$("#attachmentTable").append(tr);	
			//link = link+arrfileId[i]+","+arrfileName[i]+";";
		} 
	...
	function delRow(fileId,fileName){
		var rName = $('input[name="resultName"]').val();
		var rIds = $('input[name="resultAttachmentIds"]').val();
		var firstnameindex = rName.indexOf(fileName);
		var firstidindex = rIds.indexOf(fileId);
		//判断文件名首次出现位置
		if(firstnameindex==0){
			rName=rName.substring(fileName.length+1,rName.length);
		}else if(firstnameindex>0 && firstnameindex<rName.length){
			rName = rName.replace(new RegExp(","+fileName), "");
		}else if(firstnameindex==(rName.length-fileName.length)){
			rName=rName.substring(0,(rName.length-firstnameindex));
		}
		//判断文件id首次出现位置
		if(firstidindex==0){
			rIds=rIds.substring(fileId.length+1,rIds.length);
		}else if(firstidindex>0 && firstidindex<rIds.length){
			rIds = rIds.replace(new RegExp(","+fileId), "");
		}else if(firstidindex==(rIds.length-fileId.length)){
			rIds=rIds.substring(0,(rIds.length-firstidindex));
		}
		$('input[name="resultName"]').val(rName);
		$('input[name="resultAttachmentIds"]').val(rIds);
		$("tr ."+fileId).remove();
	};
...
<!-- 					<th width="20%">结果附件IDs: </th> -->
			...
			<c:if test="${not empty crowdsourcingAgreementDetailList}">
