...
<head>
 	...
 	function showResponse(responseText) {
		...
		if (obj.isSuccess()) {
			$.ligerMessageBox.confirm("提示信息", obj.getMessage()+",是否继续操作", function(rtn) {
				if(rtn){
					window.location.href = "${ctx}/cloud/purchase/purOrder/dir_purorder.ht";
				}else{
	...
	*/
	...
	//增加一行
	 ...
	 function setPayType(temp){
	 	$("#remark4").empty();
	 	var option="";
	 	if(temp=="立即支付"){
	 		option="<option value='立即支付'>立即支付</option>"
	 	}else{
	 		option="<option value='周结'>周结</option><option value='月结'>月结</option><option value='季结'>季结</option><option value='半年结'>半年结</option><option value='一年结'>一年结</option>"
	 	}
	 	$("#remark4").append(option);
	 }
	</script>
</head>
<body>
<div class="panel">
	...
	<div class="panel-body">
		<form id="purOrderForm" method="post" action="save_1.ht">
			<table class="table-detail" cellpadding="0" cellspacing="0" border="0" type="main">
				...
				<tr>
					<th width="20%">支付方式: </th>
					<td>
						<select id="payType" name="payType" value="${purOrder.payType}" validate="{required:false,maxlength:96}" onchange="setPayType(this.value);">
							<option value="立即支付">立即支付</option>
						...
						<input type="text" id="payType" name="payType" value="立即支付" onblur="on_type();" onfocus="on_pay(this);" class="inputText" validate="{required:false}"  />--%>			
					</td>
					<th width="20%">付款时间: </th>
					<td>
						<select id="remark4" name="remark4" value="${purOrder.remark4}" validate="{required:false,maxlength:96}" >
							<option value="立即支付">立即支付</option>
						</select>
					</td>
				</tr>
		...
		<!-- 		<tr>
			...
			<table class="table-grid table-list" cellpadding="1" cellspacing="1" id="purOrderDetail" formType="window" type="sub">
				...
				<c:forEach items="${purOrderDetailList}" var="purOrderDetailItem" varStatus="status">
				    <tr type="subdata">
				    	<td><input type="checkbox" name="check" value=""/><input type="hidden"  name="materielId" value="${purOrderDetailItem.materielId}"/></td>
				   	 	<td><input type="text" style="width: 70px;" name="materielCode"  value="${purOrderDetailItem.materielCode}"/></td>
						<td><input type="text" style="width: 70px;"  name="model"  value="${purOrderDetailItem.materielLev}"/> </td>
						<td><input type="text" style="width: 70px;" name="materielName"  value="${purOrderDetailItem.materielName}"/></td>
						<td><input type="text" style="width: 80px;"  name="attributeInfo" value="${purOrderDetailItem.attributeInfo}"/></td>
						<td><input type="text" style="width: 40px;" name="unit"  value="${purOrderDetailItem.unit}"/></td>
						<td><input type="text" name="orderNum" style="width:40px;" value="${purOrderDetailItem.orderNum}"  onblur="getSumPrice(this);" /></td>
