...
<head>
	...
	<script type="text/javascript">
			...
			if (obj.isSuccess()) {
				$.ligerMessageBox.confirm("提示信息", obj.getMessage()+",是否继续操作", function(rtn) {
					if(rtn){
						...
						window.location.href = "${ctx}/cloud/finance/finPayment/list.ht";
					}
				});
			} else {
		...
		function get_account(){
			...
			function(account) {
				$("#gatherBank").val(account.open_bank);
				$("#gatherNumber").val(account.bank_code);
			});
		}
		//付款账户
		function pay_account(){
			CommonDialog("fin_account",
					function(account) {
						...
						$("#payenterpBank").val(account.open_bank);
						$("#payenterpNumber").val(account.bank_code);
					});
		}
...
<div class="panel">
	<div class="panel-top">
		<div class="tbar-title">
		    <c:choose>
			    <c:when test="${finPayment.id !=null}">
			        <span class="tbar-label">编辑应付单据</span>
			    </c:when>
			    <c:otherwise>
...
<!-- 					</select> -->
				...
				<tr>
					<th width="20%">收款银行: </th>
					<td><input type="text" id="gatherBank" name="gatherBank" value="${finPayment.gatherBank}"  class="inputText" validate="{required:false,maxlength:192}"  />
						<a href="#" onclick="get_account()"  class="link detail">选择</a>
					</td>
					<th width="20%">收款账号: </th>
				...
				<tr>
					<th width="20%">付款银行: </th>
					<td><input type="text" id="payenterpBank" name="payenterpBank" value="${finPayment.payenterpBank}"  class="inputText" validate="{required:false,maxlength:192}"  />
					<a href="#" onclick="pay_account()"  class="link detail">选择</a>
					</td>
					<th width="20%">付款账号: </th>
