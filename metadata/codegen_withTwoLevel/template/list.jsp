	...
	<div class="panel">
		...
		<div class="panel-body">
		    ...
		    <display:table name="${classVar}List" id="${classVar}Item" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" export="true"  class="table-grid">
				...
				<#assign colName=func.convertUnderLine(col.columnName)>
				<#assign sortcolName=col.columnName>
				<#if (col.colType=="java.util.Date")>
				<display:column  title="${col.getComment()}" sortable="true" sortName="${sortcolName}">
					<fmt:formatDate value="<#noparse>${</#noparse>${classVar}Item.${colName}}" pattern="yyyy-MM-dd"/>
				</display:column>
				<#elseif (col.length > 256) >
				<display:column property="${colName}" title="${col.getComment()}" sortable="true" sortName="${sortcolName}" maxLength="80"></display:column>
				<#else>
				<display:column property="${colName}" title="${col.getComment()}" sortable="true" sortName="${sortcolName}"></display:column>
				</#if>
				</#list>
